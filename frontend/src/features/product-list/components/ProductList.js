...
import { fetchAllProdctsAsync, fetchProductsByFilterAsync, sortProductsAsync } from '../ProductSlice'
...
export default function ProductList() {
  ...
  const sortingHandler = (e, option) => {
    ...
    let newSortList ={...sortListOption, _sort:mapOption[option.name], _order:mapOrder[option.name]}
    setSortListOption(newSortList)
    dispatch(sortProductsAsync(newSortList))
  }

  useEffect(() => {
    dispatch(fetchAllProdctsAsync())
  }, [])
                                ...
                                {section.options.map((option, optionIdx) => (
                                  ...
                                  >
                                    <input
                                      ...
                                      onClick={e => filterHandler(e, section, option)}
                  ...
                  <Menu.Items className="absolute right-0 z-10 mt-2 w-40 origin-top-right rounded-md bg-white shadow-2xl ring-1 ring-black ring-opacity-5 focus:outline-none">
                    <div className="py-1">
                      {sortOptions.map((option) => (
                        <Menu.Item key={option.name} onClick={e=> sortingHandler(e,option)}>
                          {({ active }) => (
                            <a
                              href={option.href}
